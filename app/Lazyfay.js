...
function OnStart()
{
	app.MakeFolder("/sdcard/Lazyfay");
	app.MakeFolder("/sdcard/Lazyfay/Download");
	...
	actionbar = app.CreateText("Lazyfay", 1, 0.1, "Monospace,Center");
	...
	shareLazyfay = app.CreateButton("Share Lazyfay", 1.0, -0.05, "Custom");
	shareLazyfay.SetStyle("black","black",2.0,"white",1.0);
	shareLazyfay.SetOnTouch(shareLazyfay_OnTouch);
	main.AddChild(shareLazyfay);
...
function CreateDrawer()
{
	...
	var txtApp = app.CreateText( "Lazyfay",-1,-1,"Bold");
...
function bshTo_OnTouch(choose) {
	if(choose == "Yes") {
		DownloadTask("https://github.com/amsitlab/amsitlab.github.io/raw/master/dists/termux/amsitlab/binary-all/beanshell_2.04_all.deb","/sdcard/Lazyfay/Download");
	} else {
		void 0;
	}
}
...
function textrTo_OnTouch(choose) {
	if(choose == "Yes") {
		DownloadTask("https://raw.githubusercontent.com/amsitlab/textr/master/textr_1.0_all.deb","/sdcard/Lazyfay/Download");
	} else {
		void 0;
	}
}
...
function apscaTo_OnTouch(choose) {
	if(choose == "Yes") {
		DownloadTask("https://raw.githubusercontent.com/BlackHoleSecurity/apsca/master/apsca_0.1_all.deb","/sdcard/Lazyfay/Download");
	} else {
		void 0;
	}
}
...
function amoxTo_OnTouch(choose) {
	if(choose == "Yes") {
		DownloadTask("https://gitlab.com/dtlily/amox/raw/master/amox_1.0_all.deb","/sdcard/Lazyfay/Download");
	} else {
		void 0;
	}
}
...
function htmlTo_OnTouch(choose) {
	if(choose == "Yes") {
		DownloadTask("https://raw.githubusercontent.com/Cvar1984/HPB/master/html_0.1_all.deb","/sdcard/Lazyfay/Download");
	} else {
		void 0;
	}
}
...
function jadxTo_OnTouch(choose) {
	if(choose == "Yes") {
		DownloadTask("https://github.com/Lexiie/Termux-Jadx/blob/master/jadx-0.6.1_all.deb?raw=true","/sdcard/Lazyfay/Download");
	} else {
		void 0;
	}
}
...
function ginfTo_OnTouch(choose) {
	if(choose == "Yes") {
		DownloadTask("https://github.com/Gameye98/Gameye98.github.io/blob/master/package/ginf_1.0_all.deb","/sdcard/Lazyfay/Download");
	} else {
		void 0;
	}
}
...
function f4k3To_OnTouch(choose) {
	if(choose == "Yes") {
		DownloadTask("https://github.com/Gameye98/Gameye98.github.io/blob/master/package/f4k3_1.0_all.deb","/sdcard/Lazyfay/Download");
	} else {
		void 0;
	}
}
...
function intherTo_OnTouch(choose) {
	if(choose == "Yes") {
		DownloadTask("https://github.com/Gameye98/Gameye98.github.io/blob/master/package/inther_0.1_all.deb","/sdcard/Lazyfay/Download");
	} else {
		void 0;
	}
}
...
function lzmcoreTask() {
	...
	str = "<!DOCTYPE html><html><head><title>Lazyfay</title></head><body bgcolor=\"black\"><font color=\"white\"><pre>";
...
function setForumGroupData() {
	try {
		...
		xhr.open("GET","https://raw.githubusercontent.com/Gameye98/Gameye98.github.io/master/json/lazyfay_gc.json",false);
		xhr.send();
		if(xhr.status == 200) {
			app.WriteFile(lzmFolder+"/lazyfay_gc.json",xhr.response);
			data = JSON.parse(app.ReadFile(lzmFolder+"/lazyfay_gc.json"));
			...
			if(app.FileExists(lzmFolder+"/lazyfay_gc.json") != true) {
				app.WriteFile(lzmFolder+"/lazyfay_gc.json",app.ReadFile("lazyfay_gc.json"));
			}
		}
	} catch(e) {
		if(app.FileExists(lzmFolder+"/lazyfay_gc.json") != true) {
	...
	try {
		...
		if(xhr.status == 200) {
			...
			if(data["version"] == version) {
				alert("Lazyfay is already the newest version ("+version+").");
			} else {
				app.SetData("newVersion",data["url"]);
				updateNah = app.CreateYesNoDialog("A new version of Lazyfay is available!\nThe version that is currently installed is : "+version+"\n\nUpdate the Lazyfay?");
...
function shareLazyfay_OnTouch() {
	list = app.GetInstalledApps();
	for(x = 0;x < list.length;x++) {
		if(list[x].sourceDir.match(/sec.blackhole.lazyfay/g)) {
			app.CopyFile(list[x].sourceDir, "/sdcard/Lazyfay/Lazyfay.apk");
		}
	}
	app.SendFile("/sdcard/Lazyfay/Lazyfay.apk","Lazyfay.apk","Send File");
}
